...
export class SingleDamagedDevice extends Component {
    constructor(props) {
        super(props);       
        ...
        this.state = { verified: '', user_uniqid: '', device_details:[], damageDetails:[]};
    ...
    getDeviceDetails (deviceTypeUniqid, deviceUniqid, deviceSerialNumber, useParam){
        const headers = {
        ...
        .then(res => {                               
        let deviceArray = res.data;
        deviceArray['device_uniqid'] =  deviceUniqid  
        deviceArray['serial_number'] =  deviceSerialNumber  
            this.setState({
                device_details: deviceArray
            })
    ...
    componentDidMount() {
       ...
       axios.get("http://127.0.0.1:8000/inventory/lead/single/damaged/device/"+uniqid.split(":")[1], { headers: headers })
            .then(res => {
                const verified = res.data;
                // console.log(res.data)
                this.setState({
                    damageDetails: verified[0]
                })
                console.log(this.state.damageDetails['explained_issue'])
                
                axios.get("http://127.0.0.1:8000/inventory/user/single/device/"+res.data[0]['device_uniqid'], { headers: headers })
                .then(res1 => {
                    const devices = res1.data;

                    console.log(devices)
                    this.getDeviceDetails(devices['deviceTypeUniqid'], devices['uniqid'], devices['serial_number'], "myDevices")
    ...
    render() {
        ...
        const { verified, device_details, damageDetails } = this.state
console.log(damageDetails[0])
        return (
            <div>
                    <Headers />
                    <Sidebar/>               
  ...
  <div className="card" style={width400}>
    ...
    <div className="card-body">
      <h4 className="card-title">{device_details['name']}</h4>
      <p className="card-text">
        Serial Number: {device_details['serial_number']}</p>
      <p className="card-text">
        Explained Isuue: {damageDetails['explained_issue']}</p>
...
export default SingleDamagedDevice